...
'''
def fetch_session_order():
    # Creates a new cart first if the cart never existed
    if 'orderID' not in session:
        session['orderID'] = gourmetSystem.create_order()
    else:
        # Check the current cookie is valid
        try:
            gourmetSystem.get_order(session['orderID'])
        except KeyError:
            session['orderID'] = gourmetSystem.create_order()
    return gourmetSystem.get_order(session['orderID'])
'''
